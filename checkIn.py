...
def f_Check_In(vcon):
    ...
    if qtd_regs >= 2:
        ...
        vsqlU_2 = "UPDATE tb_reservas SET Status='A' WHERE Cpf = '"+cpf+"' AND Id = "+str(id_user)+"" # Comando UPDATE (Querry)
        f_checkIn_sql(vcon, vsqlU_2) # Chamando a função de Check-In

    else:

        vsqlU_1 = "UPDATE tb_reservas SET Status='A' WHERE Cpf = '"+cpf+"'" # Comando UPDATE (Querry)
        f_checkIn_sql(vcon, vsqlU_1) # Chamando a função de Check-In
